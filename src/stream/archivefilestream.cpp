...
ArchiveFileStream::ArchiveFileStream(const std::string& filename)
  : MemoryStream( filename ), m_isOpen( false )
{
  ...
  logger()->trace( L4CXX_LOCATION,
                   "Decomposed '%s': ZIP '%s', File '%s', using %s",
                   filename,
                   zipPath.string(),
                   zipFilePath.string(),
