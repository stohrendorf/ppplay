...
WavAudioOutput::WavAudioOutput(const AbstractAudioSource::WeakPtr& src, const std::string& filename)
  : AbstractAudioOutput( src ), m_file(), m_filename( filename ), m_encoderThread(), m_paused( true ), m_mutex()
{
...
int WavAudioOutput::internal_init(int desiredFrq)
{
  ...
  struct
  {
    ...
    { 'R', 'I', 'F', 'F' }, 0,
    { 'W', 'A', 'V', 'E' }, { 'f', 'm', 't', ' ' }, 16, 1, 2, static_cast<uint32_t>(desiredFrq),
    uint32_t( desiredFrq * sizeof( BasicSampleFrame ) ), sizeof( BasicSampleFrame ), 16,
    { 'd', 'a', 't', 'a' }, 0
  };
