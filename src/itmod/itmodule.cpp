...
bool updateEnvelope(SEnvelope& slaveEnvelope, const Envelope& insEnvelope, bool noteOff)
{
  ...
  const auto dt = int( insEnvelope.points[slaveEnvelope.nextPointIndex].tick )
    - insEnvelope.points[slaveEnvelope.nextPointIndex - 1].tick;
  BOOST_ASSERT( dt >= 0 );
  const auto dy =
    int( insEnvelope.points[slaveEnvelope.nextPointIndex].y ) - insEnvelope.points[slaveEnvelope.nextPointIndex - 1].y;
  if( dt == 0 )
  {
    slaveEnvelope.value.setStepSize( dy, 1 );
  }
  else
  {
    slaveEnvelope.value.setStepSize( dy, dt );
  }
  return true;
}
...
void ItModule::pitchSlideDown(SlaveChannel& slave, uint16_t val)
{
  ...
  {
    return slave.pitchSlideDownAmiga( val );
  }
  else
  {
    return slave.pitchSlideDownLinear( val );
  }
}
...
void ItModule::pitchSlideUp(HostChannel& host, uint16_t val)
{
  ...
  {
    return host.getSlave()->pitchSlideUpLinear( val );
  }
}
...
std::shared_ptr<AbstractModule> ItModule::factory(Stream* stream,
                                                  uint32_t frequency,
                                                  int maxRpt,
                                                  ppp::Sample::Interpolation inter)
{
  ...
  *stream >> result->m_header;
  ...
  for( int i = 0; i < result->m_header.smpNum; ++i )
  {
    if( (result->m_samples[i]->header.flg & ItSampleHeader::FlgWithHeader) != 0
      && result->m_samples[i]->header.samplePointer != 0 )
    {
...
#endif
      {
        result->noConstMetaInfo().trackerInfo =
          stringFmt( "Schism Tracker %04d-%02d-%02d", version.tm_year + 1900, version.tm_mon + 1,
                     version.tm_mday );
      }
      else
      {
  ...
  case 0x7000:
    ...
    {
      result->noConstMetaInfo().trackerInfo =
        stringFmt( "ITMCK %d.%d.%d", (cwtV >> 8) & 0xf, (cwtV >> 4) & 0xf, cwtV & 0xf );
    }
    break;
  case 0x8000:
  ...
  case 0xc000:
    result->noConstMetaInfo().trackerInfo =
      stringFmt( "ChickDune ChipTune Tracker %X.%02X", (cwtV >> 8) & 0xf, cwtV & 0xff );
    break;
  case 0xd000:
    if( cwtV == 0xdaeb )
    {
  ...
  while( uint8_t cellHeader = *m_patternDataPtr++ )
  {
    ...
    else
    {
      ...
      else if( host->getSlave() != nullptr && (host->getSlave()->flags & SCFLG_ON) != 0 )
      {
        host->channelState.noteTriggered = true;
        host->channelState.cell +=
          stringFmt( "%s%d ", ppp::NoteNames[host->patternNote % 12], host->patternNote / 12 + 0 );
      }
      else
      {
        host->channelState.noteTriggered = false;
    ...
    {
      host->channelState.cell +=
        stringFmt( "%c%02X", char( 'A' + (host->patternFx & 0x1f) - 1 ), int( host->patternFxParam ) );
    }
...
void ItModule::updateSamples()
{
  for( auto& slave: m_slaves )
  {
    ...
    if( (slave.flags & SCFLG_RECALC_VOL) != 0 )
    {
      ...
      auto totalVolume =
        uint32_t( slave.effectiveVolume ) * slave.channelVolume * slave.sampleVolume / 16u * state().globalVolume
          / 128u;
...
void ItModule::updateInstruments()
{
  for( auto& slave: m_slaves )
  {
    ...
    if( slave.ins != 0 )
    {
      const auto isNoteOff = (slave.flags & SCFLG_NOTE_OFF) != 0;

      if( (slave.flags & SCFLG_PITCH_ENV) != 0
        && !updateEnvelope( slave.ptEnvelope, slave.insOffs->pitchEnv, isNoteOff ) )
      {
        slave.flags &= ~SCFLG_PITCH_ENV;
      }

      if( slave.insOffs->pitchEnv.hasFilter() )
      {
        slave.envFilterCutoff = slave.ptEnvelope.value.trunc() * 256 / 64 + 128; // Range 0 -> 256
        BOOST_ASSERT( slave.envFilterCutoff >= 0 && slave.envFilterCutoff <= 256 );
        slave.flags |= SCFLG_RECALC_FINAL_VOL;
      }
      else
      {
        const auto delta = slave.ptEnvelope.value.trunc() * 256 / 8;
        BOOST_ASSERT( delta >= -1024 && delta <= 1024 );
        if( delta < 0 )
        {
          slave.pitchSlideDownLinear( -delta );
        }
        else if( delta > 0 )
        {
          slave.pitchSlideUpLinear( delta );
        }
      }

      if( (slave.flags & SCFLG_PAN_ENV) != 0 )
      {
        slave.flags |= SCFLG_RECALC_PAN;

        if( !updateEnvelope( slave.pEnvelope, slave.insOffs->panEnv, isNoteOff ) )
        {
          slave.flags &= ~SCFLG_PAN_ENV;
        }
      }

      if( (slave.flags & SCFLG_VOL_ENV) != 0 )
      {
        slave.flags |= SCFLG_RECALC_VOL;

        if( updateEnvelope( slave.vEnvelope, slave.insOffs->volEnv, isNoteOff ) )
        {
          if( (slave.flags & SCFLG_FADEOUT) != 0 || (isNoteOff && slave.insOffs->volEnv.hasGlobalLoop()) )
          {
            slave.doFadeOut();
          }

          slave.recalculate( state().globalVolume );
          continue;
        }
        else
        {
          // Envelope turned off...
          slave.flags &= ~SCFLG_VOL_ENV;
      ...
      {
        if( (slave.flags & SCFLG_FADEOUT) != 0 || (slave.flags & SCFLG_NOTE_OFF) != 0 )
        {
...
void ItModule::midiTranslateParametrized(HostChannel* host, SlaveChannel* slave, const MidiMacro& macro)
{
  ...
  for( const char chr: macro )
  {
    ...
    else if( chr == 'v' )
    {
      ...
      {
        auto tmp =
          static_cast<uint8_t>(uint32_t( slave->effectiveBaseVolume ) * state().globalVolume * slave->channelVolume / 16
            * slave->sampleVolume
            / 32768);
  ...
  if( (host.cellMask & (HCFLG_MSK_NOTE | HCFLG_MSK_INS)) != 0 )
  {
    ...
    else if( allocateChannel( host ) )
    {
      ...
      host.getSlave()->frequency = host.getSlave()->smpOffs->header.c5speed
        * std::pow( 2.0f, host.effectiveNote / 12.0f - 5 );
...
void ItModule::initCommandN(HostChannel& host)
{
  ...
  uint8_t volume = host.channelVolume;
  if( loNybble == 0x0f )
  {
    host.channelState.fxDesc = ppp::fxdesc::ChannelVolSlideUp;
...
void ItModule::initCommandO(HostChannel& host)
{
  ...
  slave->loopDirBackward = false;
  BOOST_ASSERT( slave->sampleOffset.trunc() >= 0 && slave->sampleOffset.trunc() < m_samples[slave->smp]->length()
                  && slave->sampleOffset.trunc() < slave->loopEnd );
}
...
AllocateHandleNNA:
      if( host.getSlave()->effectiveBaseVolume != 0 && host.getSlave()->channelVolume != 0
...
SlaveChannel* ItModule::allocateChannelInstrument(SlaveChannel* slave,
                                                  HostChannel& host,
                                                  const ItInstrument& instrument)
{
...
void ItModule::M32MixHandler(MixerFrameBuffer& mixBuffer, bool preprocess)
{
  ...
  for( auto& slave: m_slaves )
  {
    ...
    if( (slaveFlags & SCFLG_FREQ_CHANGE) != 0 )
    {
      ...
      slave.sampleOffset.setStepSize( slave.frequency, frequency() );
    }
...

    {
      ...
      {
        slave.filterL
             .update( frequency(), (slave.filterCutoff & 0x7fu) * slave.envFilterCutoff, slave.filterResonance );
        slave.filterR
             .update( frequency(), (slave.filterCutoff & 0x7fu) * slave.envFilterCutoff, slave.filterResonance );
