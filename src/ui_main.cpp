...
UIMain::UIMain(ppg::Widget* parent): Widget(parent)
{
	...
	m_position->setWidth(parent->getWidth()-4);
	...
	m_playbackInfo->setWidth(getWidth()-4);
	...
	m_volBar->setPosition((getWidth()-m_volBar->length())/2, 4);
	m_volBar->show();
	for(size_t i=0; i<m_chanInfos.size(); i++) {
		m_chanInfos[i] = new ppg::Label(this);
		m_chanInfos[i]->setWidth(getWidth()-4);
	...
	for(size_t i=0; i<m_chanCells.size(); i++) {
		...
		m_chanCells[i]->setWidth(getWidth() - 4);
	...
	m_trackerInfo->setPosition(2, getBottom() - 1);
	m_trackerInfo->setWidth(getWidth() - 4);
	...
	m_modTitle->setWidth(getWidth() - 4);
