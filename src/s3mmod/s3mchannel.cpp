 ...
 */
constexpr uint16_t Periods[12] =
  { 1712 << 4, 1616 << 4, 1524 << 4, 1440 << 4, 1356 << 4, 1280 << 4, 1208 << 4, 1140 << 4, 1076 << 4, 1016 << 4,
    960 << 4,
    907 << 4 };
...
inline uint8_t periodToNoteOffset(uint16_t per, uint16_t c4spd, uint16_t finetune = 8363)
{
  return static_cast<uint8_t>(std::round(
    -12 * std::log2( static_cast<float>(per) * c4spd / (finetune * Periods[0]) ) ));
}
...
S3mChannel::S3mChannel(S3mModule* const module)
  :
  m_note( 0xff ), m_lastFxByte( 0 ), m_lastVibratoData( 0 ), m_lastPortaSpeed( 0 ), m_tremorVolume( 0 )
  , m_currentVolume( 0 ), m_realVolume( 0 ), m_baseVolume( 0 )
  , m_tremorMute( false ), m_retrigCount( 0 ), m_zeroVolCounter( 3 ), m_module( module ), m_basePeriod( 0 )
  , m_realPeriod( 0 ), m_portaTargetPeriod( 0 ), m_vibratoPhase( 0 )
  , m_vibratoWaveform( 0 ), m_tremoloPhase( 0 ), m_tremoloWaveform( 0 ), m_countdown( 0 ), m_tremorCounter( 0 )
  , m_c2spd( 0 ), m_glissando( false ), m_currentCell(
...
void S3mChannel::update(const S3mCell& cell, bool patDelay, bool estimateOnly)
{
  ...
  if( m_module->state().tick == 0 )
  {
    ...
    if( m_currentCell->note() != s3mEmptyNote || m_currentCell->instrument() != s3mEmptyInstr
      || m_currentCell->volume() != s3mEmptyVolume ||
    ...
    {
      if( m_currentVolume == 0 && m_currentCell->volume() == 0 && m_currentCell->instrument() == s3mEmptyInstr
        && m_currentCell->note() == s3mEmptyNote )
      {
        m_zeroVolCounter--;
...
void S3mChannel::mixTick(const MixerFrameBufferPtr& mixBuffer)
{
  ...
  m_stepper.setStepSize( 8363 * 1712 / m_realPeriod, m_module->frequency() );
  recalcVolume();
...
void S3mChannel::playNote()
{
  ...
  if( m_currentCell->note() != s3mEmptyNote )
  {
    ...
    else if( currentSample() )
    {
      ...
      if( (m_basePeriod == 0 || m_portaTargetPeriod == 0)
        || (m_currentCell->effect() != s3mFxPorta && m_currentCell->effect() != s3mFxPortaVolSlide) )
      {
        m_realPeriod = m_basePeriod = m_portaTargetPeriod;
