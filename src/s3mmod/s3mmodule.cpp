...
S3mModule::S3mModule(int maxRpt, Sample::Interpolation inter)
  : AbstractModule( maxRpt, inter ), m_breakRow( ~0 ), m_breakOrder( ~0 ), m_patLoopRow( -1 ), m_patLoopCount( -1 )
  , m_patDelayCount( -1 ), m_customData( false )
  , m_samples( 256 ), m_patterns( 256 ), m_channels(), m_usedChannels( 0 ), m_amigaLimits( false ), m_fastVolSlides(
    false ), m_st2Vibrato( false )
  , m_zeroVolOpt( false )
{
  try
  {
    ...
    {
      chan = std::make_unique<S3mChannel>( this );
    }
  }
...
bool S3mModule::load(Stream* stream)
{
  try
  {
    ...
    *stream >> s3mHdr;
    ...
    noConstMetaInfo().trackerInfo +=
      stringFmt( "%x.%02x", (s3mHdr.createdWith >> 8) & 0x0f, s3mHdr.createdWith & 0xff );
    ...
    for( int i = 0; i < s3mHdr.smpNum; i++ )
    {
      ...
      *stream >> pp;
      ...
      m_samples[i] = std::make_unique<S3mSample>();
    ...
    for( int i = 0; i < s3mHdr.patNum; i++ )
    {
      ...
      *stream >> pp;
      ...
      auto pat = std::make_unique<S3mPattern>();
      if( !pat->load( stream, pp * 16 ) )
      {
        return false;
      }
      m_patterns[i] = std::move( pat );
      if( !stream->good() )
      {
        return false;
      }
    ...
    for( int i = 0; i < 32; i++ )
    {
      m_channels[i] = std::make_unique<S3mChannel>( this );
      const auto& s3mChan = m_channels[i];
...
{
  const auto& x = m_channels.at( idx );
  if( !x )
  {
    return ChannelState();
  }
...
std::shared_ptr<AbstractModule> S3mModule::factory(Stream* stream,
                                                   uint32_t frequency,
                                                   int maxRpt,
...
const std::unique_ptr<S3mSample>& S3mModule::sampleAt(size_t idx) const
{
...
const std::unique_ptr<S3mPattern>& S3mModule::getPattern(size_t idx) const
{
  if( idx >= m_patterns.size() || m_patterns[idx] == nullptr )
  {
    static std::unique_ptr<S3mPattern> emptyPattern = std::make_unique<S3mPattern>();
    return emptyPattern;
  }
