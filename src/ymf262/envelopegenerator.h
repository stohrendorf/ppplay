...
{
	static constexpr double* INFINITY = nullptr;
	enum class Stage
	{
...
public:
	...
	void setAtennuation( int f_number, int block, int ksl ) {
		...
		switch( ksl ) {
			...
			case 1:
				// ~3 dB/Octave
				m_attenuation = OperatorData.ksl3dBtable[hi4bits][block];
				break;
			case 2:
				// ~1.5 dB/Octave
				m_attenuation = OperatorData.ksl3dBtable[hi4bits][block] / 2;
				break;
			case 3:
				// ~6 dB/Octave
				m_attenuation = OperatorData.ksl3dBtable[hi4bits][block] * 2;
...
public:
	...
	void keyOn() {
		...
		// 2 ^ x = -envelope ->
		// x = log2(-envelope); ->
		double xCurrent = OperatorData.log2( -m_envelope );
...
private:
	static constexpr double dBtoX( double dB ) {
		return OperatorData.log2( -dB );
	}

	static constexpr double percentageToDB( double percentage ) {
		return std::log10( percentage ) * 10d;
	}

	static constexpr double percentageToX( double percentage ) {
