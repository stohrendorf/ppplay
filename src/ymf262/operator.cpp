...
{
  uint8_t
    am1_vib1_egt1_ksr1_mult4 = m_opl->readReg( m_operatorBaseAddress + Operator::AM1_VIB1_EGT1_KSR1_MULT4_Offset );
...
int16_t Operator::handleTopCymbal(uint8_t ws)
{
  ...
  const uint16_t
    phaseBit = (((hhPhase & 0x88) ^ ((hhPhase << 5) & 0x80)) | ((hhPhase ^ (hhPhase << 2)) & 0x20)) ? 0x02 : 0x00;
...
{
  const uint16_t
    phaseBit = (((m_phase & 0x88) ^ ((m_phase << 5) & 0x80)) | ((m_phase ^ (m_phase << 2)) & 0x20)) ? 0x02 : 0x00;
...
Operator::Operator(Opl3* opl, int baseAddress)
  : m_opl( opl ), m_operatorBaseAddress( baseAddress ), m_phaseGenerator( opl ), m_envelopeGenerator( opl )
{
}
