...
bool ChscPlayer::load(const std::string &filename, const CFileProvider &fp)
{
  ...
  for(i=0;i<51;i++) m_song[i] = f->readInt(1);	// load tracklist
...
bool ChscPlayer::update()
{
  ...
  for (chan=0;chan<9;chan++) {			// handle all channels
    ...
    case 0:								// global effect
       ...
       */
      switch(eff_op) {
      case 1: m_pattbreak++; break;	// jump to next pattern
      case 3: m_fadein = 31; break;	// fade in (divided by 2)
      case 5: m_mode6 = 1; break;	// 6 voice mode on
      case 6: m_mode6 = 0; break;	// 6 voice mode off
      }
      break;
    case 0x20:
