    ...
    for( int i = 0; i < 9; i++ )
    {
        ...
        { // track enabled
            m_channels[i].speed = LE_WORD(
                reinterpret_cast<const uint16_t*>(m_fileData.data() + LE_WORD(&tpoin[subsong.get_value_or(m_currentSubSong)].ptr[i])));
            m_channels[i].patternData = reinterpret_cast<const uint16_t*>(m_fileData.data() + LE_WORD(&tpoin[subsong.get_value_or(m_currentSubSong)].ptr[i]) +
                                                                          2);
        }
        else
        { // track disabled
...
void D00Player::setvolume(uint8_t chan)
{
    ...
    {
        getOpl()->writeReg(0x40 + op, static_cast<uint8_t>(63 - ((63 - m_channels[chan].modulatorVolume) / 63.0) * (63 - m_channels[chan].volume)) +
