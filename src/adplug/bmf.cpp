...
const unsigned short CxadbmfPlayer::bmf_notes[12] = { 0x157, 0x16B, 0x181,
                                                      0x198, 0x1B0, 0x1CA,
                                                      0x1E5, 0x202, 0x220,
                                                      0x241, 0x263, 0x287 };

/* for 1.1 */
const unsigned short CxadbmfPlayer::bmf_notes_2[12] = { 0x159, 0x16D, 0x183,
                                                        0x19A, 0x1B2, 0x1CC,
                                                        0x1E8, 0x205, 0x223,
                                                        0x244, 0x267, 0x28B };
...
CPlayer *CxadbmfPlayer::factory() { return new CxadbmfPlayer(); }
...
bool CxadbmfPlayer::xadplayer_load() {
  ...
  if (bmf.version > BMF0_9B) {
    ...
    for (i = 0; i < 32; i++) {
      bmf.instruments[i].name[0] = 0;
      memcpy(bmf.instruments[tune[ptr]].data, &tune[ptr + 2],
             13); // bug no.1 (no instrument-table-end detection)
      ptr += 15;
    }
  }
...
float CxadbmfPlayer::xadplayer_getrefresh() { return bmf.timer; }
...
unsigned int CxadbmfPlayer::xadplayer_getinstruments() { return 32; }
...
int CxadbmfPlayer::__bmf_convert_stream(unsigned char *stream, int channel) {
  ...
  while (true) {
    ...
    AdPlug_LogWrite(
        ...
        bmf.streams[channel][pos].note, bmf.streams[channel][pos].delay,
        bmf.streams[channel][pos].volume, bmf.streams[channel][pos].instrument,
        bmf.streams[channel][pos].cmd, bmf.streams[channel][pos].cmd_data);
