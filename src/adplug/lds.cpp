...
bool CldsPlayer::update()
{
    ...
    if(m_fadeonoff) {
        if(m_fadeonoff <= 128) {
            ...
            if(((m_allvolume + (0x100 - m_fadeonoff)) & 0xff) <= m_mainvolume)
                ...
                if(comword) {
                    if(comhi == 0x80)
                        ...
                        case 0xf9:
                            ...
                            m_jumping = 1;
                            if(m_jumppos < m_posplay) m_songlooped = true;
                            break;
                        case 0xf8:
                            c->lasttune = 0;
                            break;
                        ...
                        case 0xf4:
                            ...
                            break;
                        case 0xf3:
                            if(!m_hardfade) m_fadeonoff = comlo;
                            break;
                        case 0xf2:
                            c->trmstay = comlo;
                            break;
...
void CldsPlayer::rewind(int)
{
    ...
    m_playing = true;
    m_songlooped = false;
    m_jumping = m_fadeonoff = m_allvolume = m_hardfade = m_pattplay = m_posplay = m_jumppos = m_mainvolume = 0;
...
void CldsPlayer::playsound(int inst_number, int channel_number, int tunehigh)
{
    ...
    if((i->feedback & 1) == 1 && m_allvolume != 0) {
        slotView.modulator().setTotalLevel((((c->volmod & 0x3f) * m_allvolume) >> 8) ^ 0x3f);
    ...
    if(m_allvolume != 0) {
        slotView.carrier().setTotalLevel((((c->volcar & 0x3f) * m_allvolume) >> 8) ^ 0x3f);
    ...
    slotView.setOutput(i->feedback&0x80, i->feedback&0x40, i->feedback&0x20, i->feedback&0x10);
