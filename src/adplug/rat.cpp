...
void CxadratPlayer::xadplayer_update() {
  ...
  for (i = 0; i < rat.hdr.numchan; i++) {
    memcpy(&event, &rat.tracks[rat.order[rat.order_pos]][rat.pattern_pos][i],
           sizeof(rat_event));
#ifdef DEBUG
    AdPlug_LogWrite("order %02X, pattern %02X, row %02X, channel %02X, event "
                    "%02X %02X %02X %02X %02X:\n",
                    rat.order_pos, rat.order[rat.order_pos], rat.pattern_pos, i,
                    event.note, event.instrument, event.volume, event.fx,
                    event.fxp);
#endif
...
float CxadratPlayer::xadplayer_getrefresh() { return 60.0f; }
