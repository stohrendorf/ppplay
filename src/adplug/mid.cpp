...
CPlayer *CmidPlayer::factory()
{
    return new CmidPlayer();
}

CmidPlayer::CmidPlayer()
    : CPlayer(), m_author(&m_emptystr), m_title(&m_emptystr), m_remarks(&m_emptystr),
      m_emptystr('\0'), m_data()
{
}
...
unsigned long CmidPlayer::getnexti(size_t num)
{
    unsigned long v = 0;
...
unsigned long CmidPlayer::getnext(size_t num)
{
    unsigned long v=0;
...
bool CmidPlayer::load_sierra_ins(const std::string &fname, const CFileProvider &fp)
{
    ...
    fp.close(f);
    memcpy(m_sMyInsBank, m_myInsBank, 128 * 14);
    return true;
}
...
void CmidPlayer::sierra_next_section()
{
    ...
    while (i!=0xff)
    {
        ...
        m_tracks[m_currentTrack].spos += (getnext(1) << 8) + 4;	//4 best usually +3? not 0,1,2 or 5
        ...
        midiprintf ("track %d starts at %lx\n",m_currentTrack,m_tracks[m_currentTrack].spos);
    ...
    if ((m_adlibStyle&SIERRA_STYLE)==0)  //sierra likes it loud!
    {
        ...
        if ((m_adlibStyle&LUCAS_STYLE)!=0) {
            if ((getOpl()->readReg(0xc0+voice)&1)==1)
                getOpl()->writeReg(0x40+adlib_opadd[voice], (unsigned char)((63-vol) |
                                                                            (getOpl()->readReg(0x40+adlib_opadd[voice])&0xc0)));
            getOpl()->writeReg(0x43+adlib_opadd[voice], (unsigned char)((63-vol) |
                                                                        (getOpl()->readReg(0x43+adlib_opadd[voice])&0xc0)));
        }
        else {
            if ((getOpl()->readReg(0xc0+voice)&1)==1)
                getOpl()->writeReg(0x40+adlib_opadd[voice], (unsigned char)((63-vol) | (getOpl()->readReg(0x40+adlib_opadd[voice])&0xc0)));
            getOpl()->writeReg(0x43+adlib_opadd[voice], (unsigned char)((63-vol) | (getOpl()->readReg(0x43+adlib_opadd[voice])&0xc0)));
        }
    }
}
...
void CmidPlayer::midi_fm_playnote(int voice, int note, int volume)
{
    ...
    static constexpr int fnums[] = { 0x157, 0x16b, 0x181, 0x198, 0x1b0, 0x1ca, 0x1e5, 0x202, 0x220, 0x241, 0x263, 0x287 };
    ...
    auto c = ((freq&0x300) >> 8)+((oct&7)<<2) + (m_melodicMode || voice < 6 ? (1<<5) : 0);
...
void CmidPlayer::midi_fm_endnote(int voice)
{
    ...
    getOpl()->writeReg(0xb0+voice,(unsigned char)(getOpl()->readReg(0xb0+voice)&(255-32)));
}
    ...
    while (m_iwait==0 && ret==1)
    {
        for (m_currentTrack=0; m_currentTrack<16; m_currentTrack++)
                    ...
                    m_tracks[m_currentTrack].pos < m_tracks[m_currentTrack].tend)
            {
                ...
                if (v<0x80) {v=m_tracks[m_currentTrack].pv; m_dataPos--;}
                ...
                case 0x90: /*note on*/
                    ...
                    if (m_ch[c].on!=0)
                    {
                        ...
                        if (vel!=0 && m_ch[c].inum>=0 && m_ch[c].inum<128) {
                            if (m_melodicMode || c < 12) // 11 == bass drum, handled like a normal instrument, on == channel 6 thanks to percussion_map[] above
                            ...
                            if (m_adlibStyle & MIDI_STYLE) {
                                ...
                                if ((m_adlibStyle&LUCAS_STYLE)!=0) nv*=2;
                                if (nv>127) nv=127;
                            ...
                            midi_fm_playnote(on,note+m_ch[c].nshift,nv*2); // sets freq in rhythm mode
                            m_chp[on][0]=c;
                            m_chp[on][1]=note;
                            m_chp[on][2]=0;
                            ...
                            if(!m_melodicMode && c >= 11) {
                                ...
                                getOpl()->writeReg(0xbd, getOpl()->readReg(0xbd) & ~(0x10 >> (c - 11)));
                ...
                */
                ...
                case 0xb0: /*control change .. pitch bend? */
                    ...
                    case 0x63:
                        if (m_adlibStyle & CMF_STYLE) {
                            ...
                            //   2 == AM on
                            //   3 == AM+VIB on
                            getOpl()->writeReg(0xbd, (getOpl()->readReg(0xbd) & ~0xC0) | (vel << 6));
                ...
                case 0xf0:
                    ...
                    case 0xf7: /*sysex*/
                        ...
                        if (datalook(m_dataPos)==0x7d && datalook(m_dataPos+1)==0x10 && datalook(m_dataPos+2)<16)
                        {
                            ...
                            m_ch[c].ins[2]=(unsigned char)(0xff-(((getnext(1)<<4)+getnext(1))&0x3f));
                            m_ch[c].ins[4]=(unsigned char)(0xff-((getnext(1)<<4)+getnext(1)));
                            m_ch[c].ins[6]=(unsigned char)(0xff-((getnext(1)<<4)+getnext(1)));
                            m_ch[c].ins[8]=(unsigned char)((getnext(1)<<4)+getnext(1));

                            m_ch[c].ins[1]=(unsigned char)((getnext(1)<<4)+getnext(1));
                            m_ch[c].ins[3]=(unsigned char)(0xff-(((getnext(1)<<4)+getnext(1))&0x3f));
                            m_ch[c].ins[5]=(unsigned char)(0xff-((getnext(1)<<4)+getnext(1)));
                            m_ch[c].ins[7]=(unsigned char)(0xff-((getnext(1)<<4)+getnext(1)));
...
((float)w/(float)deltas)*((float)msqtr/(float)1000000);
    ...
    if (m_iwait !=0 && ret==1)
    {
        ...
        m_fwait=1.0f/(((float)m_iwait/(float)m_deltas)*((float)m_msqtr/(float)1000000));
    }
    else
    ...
    for (i=0; i<16; i++)
        if (m_tracks[i].on) {
            if (m_tracks[i].pos < m_tracks[i].tend)
                midiprintf ("<%d>",m_tracks[i].iwait);
            else
                midiprintf("stop");
        }
...
void CmidPlayer::rewind(int subsong)
{
    ...
    for (int i=0; i<128; i++)
        for (int j=0; j<14; j++)
            m_myInsBank[i][j]=midi_fm_instruments[i][j];
    for (int i=0; i<16; i++)
    {
        m_ch[i].inum=0;
        for (int j=0; j<11; j++)
    ...
    case FileType::Cmf: {
        ...
        i=getnexti(2);
        if(i) m_title = (char *)m_data.data()+i;
        i=getnexti(2);
        if(i) m_author = (char *)m_data.data()+i;
        i=getnexti(2);
        if(i) m_remarks = (char *)m_data.data()+i;
        ...
        m_currentTrack=0;
        m_tracks[m_currentTrack].on=1;
        m_tracks[m_currentTrack].tend=m_data.size();  // music until the end of the file
    ...
    case FileType::AdvSierra:
        memcpy(m_myInsBank, m_sMyInsBank, 128 * 14);
        m_tins = m_stins;
        m_deltas=0x20;
        ...
        if (subsong < 0 || subsong >= m_subsongs) subsong=0;
