...
CcmfPlayer::CcmfPlayer()
    : CPlayer(), data(NULL), pInstruments(NULL), bPercussive(false),
...
bool CcmfPlayer::load(const std::string &filename, const CFileProvider &fp) {
  ...
  this->pInstruments = new SBI[
      (this->cmfHeader.iNumInstruments < 128)
          ? 128
          : this->cmfHeader
                .iNumInstruments]; // Always at least 128 available for use
...
void CcmfPlayer::cmfNoteOn(uint8_t iChannel, uint8_t iNote, uint8_t iVelocity) {
  ...
  double d =
      pow(2, ((double) iNote + ((this->chMIDI[iChannel].iPitchbend - 8192) /
