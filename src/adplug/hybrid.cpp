...
void CxadhybridPlayer::xadplayer_update() {
  ...
  // process channels
  for (i = 0; i < 9; i++) {
    unsigned char *pos = &tune[
        0xADE + (hyb.order[hyb.order_pos * 9 + i] * 64 * 2) + (patpos * 2)];
    // read event
...
float CxadhybridPlayer::xadplayer_getrefresh() { return 50.0f; }
...
unsigned int CxadhybridPlayer::xadplayer_getinstruments() { return 26; }
