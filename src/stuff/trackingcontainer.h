...
public:
	...
	template<class T> static constexpr T* getAddress(T& val) { return &val; }
	template<class T> static constexpr T* getAddress(T* val) { return val; }
...
public:
	...
	operator->() { return getAddress(current()); }
	
	typename std::remove_pointer<ConstType>::type*
	operator->() const { return getAddress(current()); }
	 ...
	 * @}
	 */
	inline TrackingContainer() noexcept : m_container(), m_cursor( DanglingPosition )
	{
	}
