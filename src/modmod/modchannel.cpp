...
ModChannel::ModChannel(ppp::mod::ModModule* parent, bool isLeftChan)
  :
  m_module( parent ), m_currentCell( new ModCell() ), m_volume( 0 ), m_physVolume( 0 ), m_finetune( 0 )
  , m_tremoloWaveform( 0 ), m_tremoloPhase( 0 ), m_vibratoWaveform( 0 )
  , m_vibratoPhase( 0 ), m_glissando( false ), m_period( 0 ), m_physPeriod( 0 ), m_portaTarget( 0 )
  , m_lastVibratoFx( 0 ), m_lastTremoloFx( 0 ), m_portaSpeed( 0 )
  , m_lastOffsetFx( 0 ), m_sampleIndex( 0 ), m_lowMask( 0 ), m_portaDirUp( false ), m_stepper( 1, 1 ), m_panning(
  isLeftChan ? 0 : 0xff ), m_state()
{
...
void ModChannel::mixTick(const MixerFrameBufferPtr& mixBuffer)
{
  ...
  m_stepper.setStepSize( FrequencyBase / m_physPeriod, m_module->frequency() );
  // TODO glissando
