...
/**
 * @defgroup ModMod ScreamTracker 3 module definitions
 ...
 * @file
 * @ingroup ModMod
 * @brief This file contains the base definitions for MOD Modules
 */
...
namespace ppp {
	...
	namespace mod {
		 ...
		 */
		enum {
			modEmptyNote    = 0xff, //!< @brief Empty note
			modKeyOffNote   = 0xfe, //!< @brief Key off note
			modEmptyInstr   = 0x00, //!< @brief Empty sample
			modEmptyVolume  = 0xff, //!< @brief Empty volume
			modEmptyCommand = 0x00, //!< @brief Empty command
			modOrderEnd     = 0xff, //!< @brief "--" Marker (End of Song)
			modOrderSkip    = 0xfe  //!< @brief "++" Marker (Skip this order)
		};
		 ...
		 */
		enum {
			modTIdScreamTracker = 0x01, //!< @brief Scream Tracker 3
			modTIdImagoOrpheus = 0x02, //!< @brief Imago Orpheus
			modTIdImpulseTracker = 0x03, //!< @brief Impulse Tracker
			modTIdSchismTracker = 0x04, //!< @brief Schism Tracker
			modTIdOpenMPT = 0x05 //!< @brief OpenMPT
		};
		 ...
		 * ::modFxVibrato (and others), where each nibble is treated separately. If there is @e own @e field mentioned, that
		 * means that this effect has its own backup variable for saving the last used effect value. See ::modFxVibVolSlide for
		 * example.
		 */
		enum {
			/**
			...
			//! @see ModChannel::aLastPortaFx
			modFxPorta        =  7,
			//! @brief (H) Vibrato
			//! @note FX Data Share (Nibbles, own field)
			//! @see ModChannel::aLastVibratoFx
			modFxVibrato      =  8,
			//! @brief (I) Tremor
			//! @note FX Data Share
			...
			modFxSetPanning   = 24,
			//! @brief (Y) Panbrello
			//! @remarks Even not originally supported, I've found an MOD with it (probably a converted IT), but it won't be implemented...
			modFxPanbrello    = 25,
