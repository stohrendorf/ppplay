...
std::shared_ptr<AbstractModule> ModModule::factory(Stream* stream,
                                                   uint32_t frequency,
                                                   int maxRpt,
...
ModModule::ModModule(int maxRpt, Sample::Interpolation inter)
  : AbstractModule( maxRpt, inter ), m_samples(), m_patterns(), m_channels(), m_patLoopRow( -1 ), m_patLoopCount( -1 )
  , m_breakRow( -1 ), m_patDelayCount( -1 )
  , m_breakOrder( ~0 )
{
}
...
bool ModModule::load(Stream* stream, int loadMode)
{
  ...
  logger()
    ->debug( L4CXX_LOCATION, "%d-channel, ID '%s', Tracker '%s'", int( meta->channels ), meta->id, meta->tracker );
